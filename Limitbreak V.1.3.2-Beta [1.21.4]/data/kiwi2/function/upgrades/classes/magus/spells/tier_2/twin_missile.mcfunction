...
execute unless score @s magus_mana_bar < @s spell_cost_current run summon item_display ^-1 ^1.5 ^1 {transformation:{left_rotation:[1f,0f,1f,0f],right_rotation:[-1.25f,-0.5f,-0f,-0f],translation:[0f,0f,0f],scale:[0.4f,0.4f,0.4f]},item:{id:"minecraft:echo_shard",count:1},Tags:["magus_homing_spell","magic_missile","magic_missile_init"],teleport_duration:1}
execute unless score @s magus_mana_bar < @s spell_cost_current run summon item_display ^1 ^1.5 ^1 {transformation:{left_rotation:[1f,0f,1f,0f],right_rotation:[-1.25f,-0.5f,-0f,-0f],translation:[0f,0f,0f],scale:[0.4f,0.4f,0.4f]},item:{id:"minecraft:echo_shard",count:1},Tags:["magus_homing_spell","magic_missile","magic_missile_init"],teleport_duration:1}
execute unless score @s magus_mana_bar < @s spell_cost_current run scoreboard players set @e[type=item_display,sort=nearest,tag=magic_missile_init,limit=2] magus_homing_scoreboard 200

execute unless score @s magus_mana_bar < @s spell_cost_current run tag @e[type=item_display,sort=nearest,tag=magic_missile_init,limit=2] remove magic_missile_init
execute unless score @s magus_mana_bar < @s spell_cost_current run title @s actionbar [{"text": "[TWIN MISSILE]","color": "#59c8f7","bold": true},{"text": " -","bold": false,"color": "red"},{"score": {"name": "@s","objective": "spell_cost_current"},"bold": false,"color": "red"},{"text": " Mana","bold": false,"color": "red"}]
